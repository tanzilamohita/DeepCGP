---
title: "HDRA Prediction Accuracy Processing BayesB"
...
```{r}
comp2.acc <- read.csv(here("HDRA/HDRA_Data/HDRA_Prediction_Accuracy", "HDRA_PredictionAccuracy_Compress_2_BayesB.csv"), row.names = 1)
colnames(comp2.acc) <- colnames(pheno.read)
comp2.acc <- comp2.acc[-1,]
print(comp2.acc)
write.csv(comp2.acc, here("HDRA/HDRA_Data/HDRA_Prediction_Accuracy",  "HDRA_PredictionAccuracyProcessed_Compress_2_BayesB.csv"), quote = F)
```
read accuracy comp_3
```{r}
comp3.acc <- read.csv(here("HDRA/HDRA_Data/HDRA_Prediction_Accuracy", "HDRA_PredictionAccuracy_Compress_3_BayesB.csv"), row.names = 1)
colnames(comp3.acc) <- colnames(pheno.read)
comp3.acc <- comp3.acc[-1,]
print(comp3.acc)
write.csv(comp3.acc, here("HDRA/HDRA_Data/HDRA_Prediction_Accuracy",  "HDRA_PredictionAccuracyProcessed_Compress_3_BayesB.csv"), quote = F)
```